...
const TrendingPage: React.FC = () => {
  ...
  const [sortBy, setSortBy] = useState<"rank" | "marketCap" | "priceChange">(
    "rank"
  );
        ...
        const tokenPromises = tokenlist.map(async (token: any) => {
          try {
            ...
            if (marketResponse.ok) {
              ...
              const marketCapNumeric = parseMarketCapToNumber(
                    ...
                    <td className="p-4">
                      <div className="text-xs space-y-1">
                        ...
                        {timeFilter !== "6h" && (
                          <div
                            className={`${
                              token.priceChangeH6 >= 0
                                ? "text-green-400"
                                : "text-red-400"
                            }`}
                          >
                            6h: {token.priceChangeH6 >= 0 ? "+" : ""}
                            {token.priceChangeH6.toFixed(2)}%
                          </div>
                        )}
                        {timeFilter !== "1h" && (
                          <div
                          ...
                          >
                            1h: {token.priceChangeH1 >= 0 ? "+" : ""}
                            {token.priceChangeH1.toFixed(2)}%
                          </div>
                        )}
                        {timeFilter !== "5m" && (
                          <div
